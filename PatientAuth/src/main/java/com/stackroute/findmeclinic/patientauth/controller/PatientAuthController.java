     ...
     public  ResponseEntity<?> loginPatient(@RequestBody Patient loginDetails) {
         try {
           ...
           //  String userPhone= loginDetails.getPatientPhoneNumber();
            ...
            // Patient patient1 = patientService.findPatientBypatientEmailAndpatientPassword(userPhone, password);
             if (patient == null) {
             ...
             if (!password.equals(fetchedPassword)) {
                 throw new PatientEmailIdAndPasswordMismatchException("Invalid login credential, Please check email and password ");
             }
             // generating token
